; (function($) {
	...
	function search()
	{
		var sendData = getSearchFormData();
		$.post("/product/getJsonList.html",sendData,function(data)
		{
			if(data)
			{
				var dataList = data.data;
				var totalNum = data.total;
				var tips =  "查询条件(" + totalNum + ")";
				$('.sort_b .sort_b_inner span').html(tips);
				
				if(!dataList)
				{
					$('#list ul').html("");
					$('.list_loading').html('<span>没有数据!</span>');
					return;
				}
				if(totalNum <= 5)
				{
				...
				for(var  i = 0;i < dataList.length;i++)
				{
					...
					html += "<li ><div class='wrap'><a class='alist' vhref='/m/product/detail.html?idproduct=" +idProduct + "'>"+
							   "<div class='list_info'><h4> <i>"+obj.idProduct+"</i>  <span>"+obj.brandName+"</span>  <span>"+obj.seriesName+"</span>   <span>"+obj.curSiteName+"</span></h4>"+
								"<h5 ><span>"+obj.type+"</span> <span>"+obj.material+"</span> <span>"+obj.color+"</span> <span>"+obj.size+"</span> <span>"+obj.status+"</span></h5>"+
								"<h5 ><span>代销价："+obj.dxPrice+"</span> <span>尚上价："+obj.ssPrice+"</span> <span>尚美价："+obj.smPrice+"</span>  </h5></div><div class='clear'></div></a></div></li>";
				}
				$('#list ul').html(html);
				tcst();
				$('.alist').openNewFrame();
        ...
        function() {
            ...
            function() {
                ...
                if (t >= ($('#list').height() - $(window).height()) && scLock == true) {
                    scLock = false;
					var sendData = getSearchFormData();
					sendData.pageIndex = now_page;
					now_page++;
                    $.post('/product/getJsonList.html',sendData,
                    function(data) {
						var dataList = data.data;
						var totalNum = data.total;
						var tips = "查询条件(" + totalNum + ")";
						...
						{
							var obj = dataList[i];
							var idProdict = obj.idProduct;
							html += "<li ><div class='wrap'><a class='alist' vhref='/m/product/detail.html?idproduct=" + idProdict + "'>" +  
									   "<div class='list_info'><h4><i>"+obj.idProduct+"</i>  <span>"+obj.brandName+"</span>  <span>"+obj.seriesName+"</span>   <span>"+obj.curSiteName+"</span></h4>"+
										"<h5 ><span>"+obj.type+"</span> <span>"+obj.material+"</span> <span>"+obj.color+"</span> <span>"+obj.size+"</span> <span>"+obj.status+"</span></h5>"+
										"<h5 ><span>代销价："+obj.dxPrice+"</span> <span>尚上价："+obj.ssPrice+"</span> <span>尚美价："+obj.smPrice+"</span>  </h5></div><div class='clear'></div></a></div></li>";
						}
    ...
    $.fn.openNewFrame = function() {
        $(this).click(function() {
            ...
            _thist = $(this).find('.list_info h4 i').html();
            if (document.getElementById('newwrap')) {
   ...
   $("#updateProduct").click(function()
   {
		...
		if(flag) //验证成功 提交到后台更新
		{
			 ...
			 $("#updateProduct").val("更新中...");
			$.post("/product/updateProduct.html",data,function(text)
			{
				$("#updateProduct").removeAttr("disabled");
				$("#updateProduct").val("更新");
				if(text)
				{
					if(text.sku)
					{
 ...
 //产品详情-品类 改变  是否需要尺寸
   $('select[name="seriesId"]').change(function() {
	   ...
	   {
		   $("#sizeInput").attr("vtype","required");
	   }
	   else
	   {
		   $("#sizeInput").removeAttr("vtype");
	   }
   ...
   //产品详情-类别改变
   $('select[name="secondHand"]').change(function() {
	   
	   $('select[name="idPurchase"]').parent().hide();
	   $('select[name="idPurchase"]').removeClass("form");
	   if($(this).val() == "1")
	   {
		   $('select[name="idSupply"]').parent().show();
		   $('select[name="idSupply"]').addClass("form");
	   }
	   if($(this).val() == "2")
	   {
		   $('select[name="idSupply"]').parent().hide();
		   $('select[name="idSupply"]').removeClass("form");
	   }
	   if($(this).val() == "-1")
	   {
		   $('select[name="idPurchase"]').parent().show();
		   $('select[name="idPurchase"]').addClass("form");
   ...
   //产品入库
   $("#product2Stock").click(function(){
	   var flag = valdateForm();
	   if(flag) //验证成功 提交到后台
		{
		   $("#product2Stock").attr("disabled","disabled");
		   $("#product2Stock").val("入库中...");
			$.post("/product/productInStock.html",data,function(text)
			{
				$("#product2Stock").removeAttr("disabled");
				$("#product2Stock").val("确定入库");
				if(text)
				{
					if(text.sku)
					{
						...
						//显示供货商
						 $('select[name="idSupply"]').parent().show();
						 $('select[name="idSupply"]').val(text.idSupply);
						 $('select[name="idSupply"]').attr("disabled","disabled");
						 
						 //设置类别为新品
						 $('select[name="secondHand"]').attr("disabled","disabled");
						 $('select[name="secondHand"]').val(text.secondHand);
						 
						 
						 //设置图片
						 $("#picImg").attr("src",text.picture);
