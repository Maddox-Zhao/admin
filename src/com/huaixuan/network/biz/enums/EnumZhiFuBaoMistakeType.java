 ...
 */
public enum EnumZhiFuBaoMistakeType {
    ONLINIE("onLine", "在第"), 
    LINE("line","行"),
    PARSEMISTAKE("parseMistake","格式解析错误,元素缺少,不匹配!!!"),
    STARTPARSELINE("startParseLineStr", "#-----------------------------------------帐务明细列表----------------------------------------"), 
    ENDPARSELINE("endParseLineStr", "#----------------------------------------帐务明细列表结束-------------------------------------"),
    ...
    COUNTERTYPE("counterType", "在线支付"),
    SUCCESSORDER("successOrder", "成功订单"),
    COUNTERTYPEMISTAKE("counterTypeMistke", "业务类型出错"),
    PAYNO("payNO", "订单号"),
    IMPORTFAIL("importFail", "导入失败错误原因："),
    RE_BANKBILLNO("rebankbillno", "银行订单号重复"),
    NOTFOUNDSTARTPARSEFILE("notFoundStartParseFile", "找不到对账文件开始解析格式"),
    NOTFOUNDENDPARSEFILE("notFoundEndParseFile", "找不到对账文件结束解析格式");
    ...
    EnumZhiFuBaoMistakeType(String code, String name) {
        this.code = code;
        this.name = name;
    }
