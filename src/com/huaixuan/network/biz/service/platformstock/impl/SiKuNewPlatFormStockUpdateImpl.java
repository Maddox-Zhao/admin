...
@Service("siKuNewPlatFormStockUpdate")
public class SiKuNewPlatFormStockUpdateImpl implements SiKuNewPlatFormStockUpdate
{

	private static final String APPKEY_HK = "100245";
	private static final String SECRET_HK = "f22ca1395294671f2119289db28e7739";
	...
	{		
		...
		searchMap.put("skuisnotnull", "sikunew_sku");
		List<StockUpdate> list =  platformStockUpdateDao.selectStockUpdateByMap(searchMap);
		for(StockUpdate su : list)
		{
			if(StringUtil.isNotBlank(su.getSikunewSku()))
			{
				canSaleStockMap.put(su.getSikunewSku(), su);
			}
		}
		...
		System.out.println(sdf.format(new Date()) + " sikunew " + type);
	}
	...
	public boolean updateSikuStock(String sikuProductId,String ourSku,int num,String type)
	{
		...
		{
			searchMap.put("type", type);
			searchMap.put("sikunewSku", sikuProductId);
			List<StockUpdate> list = platformStockUpdateDao.selectStockUpdateByMap(searchMap);
			if(list.size() > 0)
			{
		...
		else if(StringUtil.isBlank(sikuProductId))
		{
			...
			searchMap.put("type", type);
			searchMap.put("field", "sikunew_sku");
...
//		System.out.println(response);
	...
	public void updateSku2LocationByFile(Map<String,String> keyMap,String type)
	{
		 ...
		 while(it.hasNext())
		 {
			...
			pslSku.setPlatformField("sikunew_sku");
			...
			shangpinSkuMap.put("platformField", "sikunew_sku");
			if(list.size() > 0)
			{
				//更新尚品sku到本地
...
//										platformorderdetails.setTotalPrice(totalPrice);
										...
										platformorderdetails.setPtype("sikunew");
					...
					if(sikuOrderMapRealy.get(orderId) == null) //没有保存该orderid 没处理  增加orderid数 并插入表
					{
						...
						for(int index = 0; index < orderSkus.length(); index++)
						{
							...
							{
//								su.setSikunewOrderStock(num + su.getSikunewOrderStock());
								su.setSikuOrderStock(num + su.getSikuOrderStock());  //新寺库的订单数量都放入siku_order_stock，不放sikunew_order_stock,目的是方便开单时减订单数
