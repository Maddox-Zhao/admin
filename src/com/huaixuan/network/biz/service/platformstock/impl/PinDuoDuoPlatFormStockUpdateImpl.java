...
//2017-2-8下午2点左右从新获得token
@Service("PinDuoDuoPlatFormStockUpdate")
public class PinDuoDuoPlatFormStockUpdateImpl implements
		PinDuoDuoPlatFormStockUpdate {
	...
	private static final String code = "94a93b36fad7421085d3fe20dab5f4d62e302c8b";
	...
	private static final String refresh_token = "3ccf0a90d9294f9da31ec43e342a30e4365c21aa";
	...
	public void updateSku2Location() {
		...
		for (;;) {
			...
			String response = HttpRequest.sendPost(
					"http://gw-api.pinduoduo.com/api/router", map);
			
//			System.out.println(response);
			try {
				JSONObject jsonObj = new JSONObject(response);
				String goods_list_get_response_str = jsonObj
						.getString("goods_list_get_response");
				JSONObject goods_list_get_response = new JSONObject(
						goods_list_get_response_str);
				int totalNum = goods_list_get_response.getInt("total_count");
				int totalPageSize = (int) Math.floor(totalNum / page_size) + 1;

				JSONArray items = goods_list_get_response
						.getJSONArray("goods_list");

				List<PlatformSku2LocationSku> pddskuList = new ArrayList();
				for (int i = 0; i < items.length(); i++) {
					...
					for (int index = 0; index < skus.length(); index++) {
						JSONObject sku = new JSONObject(skus.get(index)
								.toString());
						...
						if ((StringUtil.isNotBlank(pddsku))
								&& (StringUtil.isNotBlank(ourSku))) {
							PlatformSku2LocationSku pslsku = new PlatformSku2LocationSku();
							pslsku.setOurSku(ourSku);
							pslsku.setPlatformField("pdd_sku");
				...
				pddSkuMap.put("platformField", "pdd_sku");
				if (pddskuList.size() > 0) {
					this.platformStockUpdateDao
							.batchUpdatePlatformSku2Location(pddSkuMap);
				}
	...
	public boolean updatePinDuoduoStock(String pddSku, String ourSku,
		...
		if (StringUtil.isBlank(ourSku)) {
			...
			searchMap.put("pddSku", pddSku);
			List<StockUpdate> list = this.platformStockUpdateDao
			...
			searchMap.put("type", type);
			searchMap.put("field", "pdd_sku");
			pddSku = this.platformStockUpdateDao
	...
	public void updateAllStock() {
		...
		serchMap.put("skuisnotnull", "pdd_sku");
		List<StockUpdate> list = this.platformStockUpdateDao
				.selectStockUpdateByMap(serchMap);
		for (StockUpdate s : list) {
			int num = s.getNowStockNum() - s.getOrderStockNum();
			if (num < 0) {
				num = 0;
			}
			updatePinDuoduoStock(s.getPddSku(), s.getSku(), num, s.getType());
			try {
				Thread.sleep(500L);
	...
	public int atuoSyncOrder() {
		...
		try {
			...
			for (int index = 0; index < orderList.length(); index++) {
				JSONObject orderInfo = new JSONObject(orderList.get(index)
						.toString());
			...
			try {
				...
				for (int j = 0; j < item_list.length(); j++) {
					...
					List<PlatFormOrderDetails> liststatus = platformOrderDetailsDao.selectList(platformorderdetails);
						...
						platformorderdetails.setPtype("pdd");
		...
		if (orderMap.size() > 0) {
			...
			while (it.hasNext()) {
				...
				if (orderMapRealy.get(orderId) == null) {
					for (String v : value) {
						...
						for (StockUpdate su : stockUpdateList) {
							Map<String, String> searchMaps = new HashMap();
							...
							if ((stockUpdateLists != null)
									&& (stockUpdateLists.size() > 0)) {
								int s = su.getNowStockNum() - su.getOrderStockNum();
								if (s <= 0) {
									for (StockUpdate ss : stockUpdateLists) {
										ss.setPddOrderStock(num+ ss.getPddOrderStock());
				...
				if (updateSuccess) {
					...
					for (String v : value) {
						String[] arr1 = v.split("_");
						String pddsku = arr1[0];
						int num = Integer.valueOf(arr1[1]).intValue();
						}
					}
				}
			}
...
//	 @PostConstruct
	public void startDealPinduoduoRefresh() {
		...
		Thread thread = new Thread(new Runnable() {
			...
			public void run() {
				while (true) {
					PinDuoDuoPlatFormStockUpdateImpl pdd = new PinDuoDuoPlatFormStockUpdateImpl();
					pdd.pinduoduoRefresh();
					try { 
