...
/**
 * @author guoyj
 * @version $Id: AliPayParseCompareFile.java,v 0.1 2010-6-7 涓嬪崍02:43:14 guoyj Exp $
 */
...
public class AliPayParseCompareFileTrans extends AbstractParseCompareFileTrans {
    ...
    protected CounterOutPintMessage getParseFile(CounterCompareReq counterCompareReq) {
        ...
        try {
            String endParseLineStr = EnumZhiFuBaoMistakeType.ENDPARSELINE.getValue(); //阿里巴巴对账文件结束对账解析格式
            ...
            while ((m = file.readLine()) != null) {
                line++;
                if (!m.equals(endParseLineStr)) //文本结束的标志   
                {
                    if (!m.equals("") && (line > 5)) //不需要读取空行,并且是在文本开始标志下一处开始读取  
                    {
                        sBuf.delete(0, sBuf.length());
                        String failDescriptionStr = ""; //导入错误原因

                        data = m.split("\\,"); //得到的每行分割数组项
                        ...
                        /********************如果导入的文件的元素不匹配的话*******************************************/
                        try {
                            payDate = data[3];//订单生成时间
                            bankType = counterCompareReq.getCompareAccountType(); //银行类型
                            bankBillNo = data[2]; //银行订单号
                            BigDecimal bg=new BigDecimal(bankBillNo);
                            inPayAmount = data[5]; //收入金额
                            bankSerialNo = data[1]; //银行流水号
                            counterType = data[9]; //业务类型
                        } catch (Exception e) {
                        ...
                        } else //否则不允许
                        {
                            ...
                            counterOutPintMessageu.getFailDescriptionListu()
                                .add(failDescriptionStr); //添加导入失败明细
                            continue;
                        }
                    }
