...
public class CategoryEditValidator extends ValangValidator{
	...
	public void validate(Object obj, Errors errors) {
		...
		if(StringUtil.isEmpty(category.getCatName())){
			...
			if(category.getCatName().length() > 60){
				...
				try {
					...
					Category categoryTemp = categoryManager.getCateInfoByCatName(categoryDb);
		...
		if(category.getSortOrder() != 0){
			if (category.getSortOrder() < 0) {
				errors.rejectValue("sortOrder", "", "类目序号不能小于0");
			}
			if (category.getSortOrder() > 999) {
				errors.rejectValue("sortOrder", "", "类目序号不能大于999");
			}
		}else{
			errors.rejectValue("sortOrder", "", "类目序号不能等于0");
		}
