...
public class Billing_Deal_DetailsUtil {
	...
	protected static Log log = LogFactory.getLog(Billing_Deal_DetailsUtil.class);
	// 订单已付款状态
	 ...
	 *            订单编码
	 * @param listItem
	 *            是否列出商品: 0（默认）：不列出订单相关商品
	 * @param sign
	 *            所有非文件字段name和value的校验值，防止重要数据被中途篡改
	 * @return
	 */
	public static String CreateDealDetailUrl(String url, String sellerUin, String dealCode, int listItem) {
	 ...
	 *            number 0,1 默认值为0。
	 * @param timeType
	 *            string 空字符串（默认） 时间类型。 空字符串（默认）:表示不使用时间筛选项 CREATE:表示timeBegin和timeEnd是下单时间
	 *            UPDATE:表示timeBegin和timeEnd是订单最后更新时间 如果timeType的类型为CREATE、UPDATE时，timeBegin和timeEnd字段必须赋值
	 * @param timeBegin
	 *            time 格式: yyyy-MM-dd HH:mm:ss 起始时间 如果字段timeType的类型为CREATE、UPDATE时，该字段必须赋值
	 * @param timeEnd
	 *            time 格式:yyyy-MM-dd HH:mm:ss 结束时间 如果字段timeType的类型为CREATE、UPDATE时，该字段必须赋值
	 * @param dealState
	 *            string 默认为空字符串 订单状态:dealState
	 * @param itemCode
	 *            string 商品编码 这里需要主要不要填写订单中的商品快照编码，是要填写商品编码 4A42403200000001004A39AB068030E3 订单中的商品快照编码
	 *            4A42403200000000004A39AB068030E3 商品编码 商品快照编码：当买家下单后，如果商家修改了商品信息，那么订单中查看的商品编码，
	 ...
	 * @return
	 */
	public static String CreateDealListUrl(TreeMap<String, String> signParams, String pageIndex, String pageSize,
			String timeType, String timeBegin, String timeEnd, String dealState, String itemCode, String itemNameKey,
			String listItem, String dealCode, String orderField, String orderDesc) {
	...
	public static String CreateDealConsignUrl(TreeMap<String, String> signParams, String dealCode,
			String logisticsName, String logisticsDesc, String logisticsCode, int arriveDays) {
	 ...
	 *            string 订单编码
	 * @param closeReason
	 *            number 填写的取消说明 0：无法联系上买家 1：买家误拍或重拍了 2：买家无诚意完成交易 3：已通过银行线下汇款 4：已通过同城见面交易 5：已通过货到付款交易 6：已通过网上银行直接汇款
	 *            7：已经缺货无法交易
	 * @return
	 */
		...
		try {
			...
			if (!errorCode.equalsIgnoreCase("0")) {
				resultMap.put("errorMessage",
						EnumBilling_Paipai_Detail.No_Permission.getValue(root.elementText("errorMessage")));// 错误信息
				return resultMap;
			} else {
				...
				while (dealList.hasNext()) {
					...
					while (deal.hasNext()) {
						...
						if (StringUtils.isNotBlank(ele.elementText("createTime"))) {
							trade.setCreateTime(DateUtils.parseDate(ele.elementText("createTime"), str));
						}
						if (StringUtils.isNotBlank(ele.elementText("payTime"))) {
							trade.setDealPayTime(DateUtils.parseDate(ele.elementText("payTime"), str));
						}
						...
						for (int i = 0; i < stockLocalCodeList.size(); i++) {
							...
							// 订单中商品信息
							order.setCode(StringUtils.isBlank(element1.getText()) ? element3.getText() : element1
									.getText());
	...
	public static void main(String[] args) throws Exception {
		...
		// String ItemUrl = CreateDealDetailUrl(DEAL_DETAIL_URL, Contants.UIN.toString(),
		// "855008924-20100804-359568958", "0");
		/**
		 * String url,String pageIndex,String pageSize, String timeType,String timeBegin,String timeEnd, String
		 * dealState, String itemCode, String itemNameKey,String listItem,String dealCode,String orderField,String
		 * orderDesc
		 */
		...
		// String ItemUrl =
		// CreateDealConsignUrl(DEAL_CONSIGN_URL,"855008924-20100804-359568958","申通快递","已发货","19291828131",2);
		// String ItemUrl = CreateDelayConsignmentTimeUrl(DELAY_CONSIGNMENT_TIME_URL,"855008924-20100804-359568958",5);
		// String ItemUrl =
		// CreateModifySellerNoteUrl(MODIFY_SELLER_NOTE_URL,"855008924-20100804-359568958","修改订单备注信息接口测试");
